...
if(isset($_POST['update'])){
	...
	if(filter_var($email, FILTER_VALIDATE_EMAIL)){
		...
		if($num_rows > 0 && $email_row == 0){
			...
			array_push($error_array, "this email already exist");
		}
		
	}else{
		array_push($error_array, "this must be valid email");
	}

	if(strlen($first_name) > 20 || strlen($first_name) < 3){
		array_push($error_array, "First name bust be betwen 20 and 3 characters");
	}

	if(strlen($last_name) > 20 || strlen($last_name) < 3){
		array_push($error_array, "Last name bust be betwen 20 and 3 characters");
	}
